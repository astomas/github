// ============================================================================
// CORRECTIONS OMS - AVEC DÉCLENCHEMENT AUTO AU ZOOM > 14
// ============================================================================
//
...
// 5. ✅ GARDE le déclenchement automatique du spider au zoom > 14
//
// ============================================================================
...
// ============================================================================
// ✅ DÉCLENCHEMENT AUTOMATIQUE DU SPIDER AU ZOOM > 14
// Version CORRIGÉE qui évite les doublons
// ============================================================================
maCarte.on('zoomend', function() {
	const zoomActuel = maCarte.getZoom();

	if (zoomActuel >= 14) {
		// À zoom >= 14 : déclenchement automatique du spider
		const markers = oms.getMarkers();
		const dejaTraites = new Set();
		const seuilDistanceSq = 1 * 1;  // nearbyDistance au carré

		markers.forEach(marker => {
			// Ignorer si déjà traité ou pas sur la carte
			...
			// Trouver tous les marqueurs qui se superposent
			markers.forEach(m => {
				...
				if (distSq <= seuilDistanceSq) {
					// Marqueur dans le groupe (superposé)
			...
			// ✅ Déclencher le spider si groupe > 1
			if (groupe.length > 1) {
				// IMPORTANT : Cette ligne déclenche automatiquement le spider
				oms.spiderfy(groupe, autresMarkers);
			}
		});
	} else {
		// À zoom < 14 : fermer les spider legs
		oms.unspiderfy();
	}
});

console.log("OMS initialisé avec nearbyDistance: 1px + déclenchement auto au zoom >= 14");
